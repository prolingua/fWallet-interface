...
export const FANTOM_CHAIN_ID =
  process.env.REACT_APP_USE === "testnet" ? 0xfa2 : 0xfa;
...
export const ErrorMessages = {
  // Bad request header.
  [ErrorCodes.ERR_BAD_REQUEST_HEADER]:
    "Invalid request header sent to the hw wallet.",

  // Unknown service class CLA received.
  [ErrorCodes.ERR_UNKNOWN_CLA]:
    "Unknown service class, please use correct service identifier.",

  // Unknown instruction arrived.
  [ErrorCodes.ERR_UNKNOWN_INS]:
    "Unknown instruction sent, please send a supported instruction.",

  // Request is not valid in the current context.
  [ErrorCodes.ERR_INVALID_STATE]:
    "Invalid instruction state, the instruction sequence did not follow the protocol.",
  ...
  [ErrorCodes.ERR_INVALID_DATA]:
    "Data sent with the request has not been recognized.",
  ...
  [ErrorCodes.ERR_REJECTED_BY_POLICY]:
    "Requested action has been denied by security policy.",

  // Device is locked.
  [ErrorCodes.ERR_DEVICE_LOCKED]:
    "Can not proceed with the instruction, please unlock the device.",
};
...
export default class FantomNano {
  ...
  async getAddress(accountId = 0, addressId = 0, confirmAddress = true) {
    ...
    return this.deriveAddress(
      this.getBip32Path(accountId, addressId),
      confirmAddress
    );
  }
  ...
  async signTransaction(accountId, addressId, tx) {
    ...
    const step1Init = async (bip32Path) => {
      ...
      // execute the call
      return this.send(CLA, INS.SIGN_TRANSACTION, p1, p2, data).then(
        (response) => {
          // extract version data
          const data = stripReturnCodeFromResponse(response);
    ...
    const step2TxTransfer = async (chunk) => {
      ...
      return await this.send(CLA, INS.SIGN_TRANSACTION, p1, p2, chunk).then(
        (response) => {
          // extract version data
          const data = stripReturnCodeFromResponse(response);
          ...
          Assert.check(
            data[0] === SIGN_STATE.COLLECT || data[0] === SIGN_STATE.FINALIZE
          );
    ...
    const step3TxFinalize = async () => {
      ...
      // send the RLP encoded transaction chunk to the device
      return await this.send(CLA, INS.SIGN_TRANSACTION, p1, p2, data).then(
        (response) => {
          // extract version data
          const data = stripReturnCodeFromResponse(response);
    ...
    for (let i = 0; i < parts; i++) {
      chunks[i] = txBuffer.slice(
        i * MAX_APDU_CHUNK_LENGTH,
        (i + 1) * MAX_APDU_CHUNK_LENGTH
      );
    }
    ...
    if (finalStatus !== SIGN_STATE.FINALIZE) {
      ...
      throw new Error(
        "Transaction data was not recognized on the Ledger device!"
      );
    }
